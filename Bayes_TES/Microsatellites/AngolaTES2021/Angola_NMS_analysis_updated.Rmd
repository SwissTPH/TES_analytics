...
for (j in 1:nloci) {
  ...
  if (is.null(dim(oldalleles))) {
    oldalleles = matrix(oldalleles, ncol=1)
  }
  ...
  for (i in 1:ncolumns) {
    newalleles[, i] = sapply(1:nrow(oldalleles), function(x) recodeallele(alleles_definitions_RR[[j]], oldalleles[x, i]))
  }
...
if (length(additional_neutral) > 0 && dim(additional_neutral)[1] > 0) {
  
  recoded_additional_neutral = matrix(0, nrow = dim(additional_neutral)[1], ncol = maxMOI * nloci)
...
* [Bayesian algorithm for molecular correction](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4576024/)

```{r}
# Plot for qq (parameter 1)
plot(state_parameters[1, ], type = 'l', xlab = "Iteration (post-burnin, thinned)", ylab = "qq")
# Plot for dposterior (parameter 2)
plot(state_parameters[2, ], type = 'l', xlab = "Iteration (post-burnin, thinned)", ylab = "dposterior")
