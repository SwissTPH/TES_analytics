generate_likelihood_diagnostics <- function(all_chains_loglikelihood, site_name) {
  ...
  loglikelihood_mcmc <- tryCatch({
    as.mcmc.list(lapply(clean_chains, function(x) mcmc(matrix(x, ncol = 1))))
  }, error = function(e) {
  ...
  varnames(loglikelihood_mcmc) <- "loglikelihood"
  
  # --- Gelman-Rubin Diagnostic ---
  gelman_result <- tryCatch({
    gelman.diag(loglikelihood_mcmc, autoburnin = FALSE, multivariate = FALSE)
  }, error = function(e) {
    message("Gelman-Rubin failed: ", e$message)
    chain_means <- sapply(clean_chains, mean, na.rm = TRUE)
    chain_vars  <- sapply(clean_chains, var, na.rm = TRUE)
  ...
  cat("Gelman-Rubin R̂ for Log-Likelihood:\n")
  print(gelman_result)
  
  # --- Effective Sample Size ---
  ess_result <- tryCatch({
    effectiveSize(loglikelihood_mcmc)
  }, error = function(e) {
  ...
  plot_base <- function(filename, expr) {
    png(file.path(site_dir, filename), width = 800, height = 600)
  ...
  plot_base("loglikelihood_traceplot.png", {
    colors <- rainbow(length(loglikelihood_mcmc))
  ...
  plot_base("gelman_loglikelihood.png", {
    gelman.plot(loglikelihood_mcmc, autoburnin = FALSE, ask = FALSE)
    title(main = paste(site_name, "- Gelman-Rubin for "), line = 2.5)
  })
  
  
  # Histogram
  plot_base("loglikelihood_histogram.png", {
    hist(unlist(clean_chains), breaks = 40, main = paste(site_name, "- Histogram of Log-Likelihood"),
  ...
  plot_base("loglikelihood_acf.png", {
    ...
    for (i in seq_along(clean_chains)) {
      acf(clean_chains[[i]], main = paste("ACF - Chain", i), lag.max = 50)
    }
  })
  
  # --- Save CSV results ---
  # try(write.csv(gelman_result$psrf, file.path(site_dir, "gelman_loglikelihood.csv")), silent = TRUE)
  # try(write.csv(data.frame(ESS = ess_result), file.path(site_dir, "ess_loglikelihood.csv")), silent = TRUE)
