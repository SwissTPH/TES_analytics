...
run_all_sites <- function(
    ...
    burn_in_frac) {
        ...
        for (site in site_names) {
            ...
            cat(sprintf("WARNING: No MCMC samples were generated for site '%s'. Skipping.\n", site))
            next
            }
            ...
            cat(sprintf("WARNING: Burn-in (%d) is >= total samples (%d) for site '%s'. Skipping analysis.\n", burn_in_samples_per_chain, num_total_samples_per_chain, site))
            next
            }
            ...
            local_sites_parameters[[site]] <- final_parameters
            local_sites_alleles0[[site]] <- final_alleles0
            local_sites_allelesf[[site]] <- final_allelesf
            local_sites_ids[[site]] <- ids
        }
    return(
    list(
      classifications = local_sites_classification,
      parameters = local_sites_parameters,
      alleles0 = local_sites_alleles0,
      allelesf = local_sites_allelesf,
      ids = local_sites_ids,
      genotypedata = genotypedata_latefailures 
    )
  )
}
