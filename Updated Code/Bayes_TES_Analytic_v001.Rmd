...
run_one_chain <- function(chain_id,
  ...
  ##### define statevector and create state 0
  ...
  recr_repeatsf <- matrix(NA, nids, nloci) # FIX: This line was added back in.
  ...
  # (The original data processing block to create state 0 is here)
  for (j in 1:nloci) {
      locus = locinames[j]
  ...
  ##### Finish initialization for the current chain
  ...
  # MCMC RUNNER FUNCTION (Closure) - This remains unchanged
  
  # IN MAIN SCRIPT, INSIDE run_one_chain
  ...
  if (!any(valid)) {
    if (should_print_debug) {
      ...
      print(head(distances, 20)) # Print the first 20 distances to inspect them
    }
    return(1) # Return the neutral value
  }
  ...
  denominators <- sapply(which(valid), function(z) {
    ...
    if (is.na(recr_allele)) {
      return(NA) # If the allele is missing, this path cannot be evaluated.
    }
    sum(
  ...
  if (length(final_ratios) == 0) {
    if (should_print_debug) {
      ...
      print(head(denominators, 20)) # This will likely show a vector of NAs
    }
    return(1) 
  }
  ...
  likelihoodratio <- sapply(1:nids, function(x) {
    ...
    # Re-assign all the modified state variables using '<<-'
  ...
  for (locus_idx in 1:nloci) {
    ...
    tempdata = full_data,
    maxMOI = maxMOI,
    frequencies_RR = frequencies_RR # Pass the current state in
  )
  ...
  # if(count %% 100 == 0) {
  #   message(sprintf("Count %d: NA ratios: %d, Min ratio: %.3f, Max ratio: %.3f",
  #                  count,
  #                  sum(is.na(likelihoodratio)),
  #                  min(likelihoodratio, na.rm=TRUE),
  #                  max(likelihoodratio, na.rm=TRUE)))
  # }    
  ...
  # --- [END of code to copy from your original loop] ---

  # message(sprintf("  PID %d: Finished Chain %d.", Sys.getpid(), chain_id))
  # 
  # message(sprintf("---------- DEBUGGING CHAIN %d ----------", chain_id))
  # message("Summary of the recorded log-likelihood trace:")
  # print(summary(state_loglikelihood))
  # 
  # # This line is the most important one. If it prints 1, our theory is correct.
  # message(sprintf("Number of unique log-likelihood values: %d", 
  #                 length(unique(na.omit(state_loglikelihood)))))
  # 
  # message("First 10 values of the trace:")
  # print(head(state_loglikelihood, 10))
  # message("--------------------------------------")
  # 
...
### =========================================================================
###      FINAL, CLEAN AUTOMATED MCMC SCRIPT
### =========================================================================
...
library(future)
library(future.apply)
library(abind) # Needed for 3D array manipulation

# --- 1. Define Core MCMC and Data Parameters ---
site_names <- unique(genotypedata_latefailures$Site)
n_chains <- 4 

# --- 2. Automated MCMC Control Parameters ---
R_HAT_THRESHOLD <- 1.01
ESS_THRESHOLD <- 400
CHUNK_SIZE <- 2000
MAX_ITERATIONS <- 10000 # Using 10k for faster testing, can increase later
BURN_IN_FRAC <- 0.25

# --- 3. Global Storage for Aggregating Results ACROSS ALL SITES ---
all_sites_classification <- list()
all_sites_parameters <- list()
all_sites_alleles0 <- list() # Added for completeness
all_sites_allelesf <- list() # Added for completeness
all_sites_ids <- list()

# --- 4. Configure Parallel Plan (Once, for efficiency) ---
plan(multisession, workers = n_chains)
message(sprintf("Parallel backend configured with %d workers.", nbrOfWorkers()))
...
for (site in site_names) {
    jobname <- site
    cat(sprintf("\n--- Starting Automated Analysis for Site: %s ---\n", site))
    
    # --- Site-Specific Data Preparation ---
    start_time_site <- Sys.time()
    ...
    while (!converged && total_iterations < MAX_ITERATIONS) {
        ...
        if (length(full_chain_results) == 0) {
            ...
            for (i in 1:n_chains) {
                ...
                # Correctly append to the i-th element of the list, not the whole list
    ...
    if (num_total_samples_per_chain == 0) {
        cat(sprintf("WARNING: No samples generated for site '%s'. Skipping analysis.\n", site))
        next
    }
    ...
    # *** CORRECTION 2: USING 'final_parameters' INSTEAD OF 'combined_parameters' ***
    ...
    end_time_site <- Sys.time()
    runtime <- end_time_site - start_time_site
    message(sprintf("Total runtime for site '%s': %s", site, runtime))
